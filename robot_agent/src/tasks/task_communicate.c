...
void task_communicate(void)
{
    ...
    if (g_task_communicate.enabled == s_TRUE)
    {
        ...
        while (g_list_send->count > 0)
        {
            ...
            {
            case s_DATA_STRUCT_TYPE_ROBOT:
                doublylinkedlist_empty(position_list); // keep only latest pheromone
            ...
            case s_DATA_STRUCT_TYPE_VICTIM:
                ...
                break;
            case s_DATA_STRUCT_TYPE_PHEROMONE:
                if (pheromone_list->count == 8)
                    doublylinkedlist_empty(pheromone_list); // keep only latest pheromone
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            //evaluate bits sent when pheromones and stream 
            if (i >= 2)
                measure_flag = 1;
            ...
            while (current_list->count != 0)
            {
                ...
                if (measure_flag == 1 && send_exceed_flag == 0)
                {
                    ...
                    if (bit_sent > 18800) // 400 bits of margin
                    {
                        send_exceed_flag = 1;
                        doublylinkedlist_empty(current_list);
                        break;
                    }
                }
